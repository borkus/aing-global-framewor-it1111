...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.post('/api/contact', async (c) => {
        try {
            ...
            return c.json({ success: false, error: 'Failed to process contact form.' }, { status: 500 });
        }
    });
    /**
    ...
    app.post('/api/subscribe', async (c) => {
        try {
            ...
            if (!email || typeof email !== 'string') {
                return c.json({ success: false, error: 'Email is required.' }, { status: 400 });
            }
            ...
            return c.json({ success: false, error: 'Failed to process subscription.' }, { status: 500 });
        }
    });
}